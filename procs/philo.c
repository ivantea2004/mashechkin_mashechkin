...
Считаем, что философы(польщователи) сидят за круглым столом.
...
user(int id)
{
...
#if USE_ORDER // Если есть флаг, то будем соблюдать порядок захвата вилок (ресурсов)
    if (left > right) {
        int tmp = left;
        left = right;
        right = tmp;
    ...
    while (!end) { // Философы едят, пока не придёт время заканчивать
                     ...
                     // происходит синхронизированно с другими захватами, что гарантируется испольованием глобального мьютекса
